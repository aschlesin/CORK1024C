...
class CalibrationCoefficients(object):
    ...
    def __init__(
         self, 
           ID=0x8B,  # ID for BPR in hexformat
    ...
    def readParoCoeffs(self):
        ...
        ParoCoeffs={}
        f=open(os.path.join(self.ConfigPath,self.parofile))
        N=0
        while True:
            try:
                N += 1
...
#    
    ...
    def readPlatinumCoeffs(self):
        ...
        if not self.ConfigPath: self.ConfigPath = os.getcwd()
        PlatinumCoeffs={}
        f=open(os.path.join(self.ConfigPath,self.platinumfile))
        N=0
        while True:
            try:
                N += 1
